...
const string strMessageMagic = "TeleBET Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 2160)
    {
      nSubsidy = 25 * COIN;
      return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight <= (8 * DAILY_BLOCKCOUNT)) //8 days high PoS
	{
		nSubsidy = MAX_MINT_PROOF_OF_STAKE;
	}
    else 
    {
		nSubsidy = MAX_MINT_PROOF_OF_STAKE2;
    }
	
    LogPrint("creation", "GetProofOfStakeReward(): create=%s nCoinAge=%d\n", FormatMoney(nSubsidy).c_str(), nCoinAge);

//    printf("GetProofOfStakeReward(): create=%s nCoinAge=%s nRewardCoinYear=%s COIN=%s\n", FormatMoney(nSubsidy).c_str(), FormatMoney(nCoinAge).c_str(), FormatMoney(nRewardCoinYear).c_str(), FormatMoney(COIN).c_str());
...
#ifdef ENABLE_WALLET
// telebet: attempt to generate suitable proof-of-stake
