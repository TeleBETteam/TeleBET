...
{
    // TeleBET: check prefix
    if(uri.scheme() != QString("telebet"))
        return false;
...
{
    // Convert telebet:// to telebet:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("telebet://"))
    {
        uri.replace(0, 12, "telebet:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TeleBET.lnk";
}
...
{
    return GetAutostartDir() / "telebet.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TeleBET-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  telebet-qt [" + tr("command-line options") + "]                     " + "\n";
